<x-layouts::app title="Create Event">
    <div class="py-12">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 bg-white p-8 border rounded shadow-md">
            <h1 class="text-2xl font-bold mb-6">เพิ่มหัวข้อกิจกรรมใหม่</h1>

            <form action="{{ route('admin.events.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                    <label class="block text-black text-sm font-bold mb-2">ชื่อหัวข้อกิจกรรม</label>
                    <input type="text" name="title" class="shadow appearance-none border rounded w-full py-2 px-3 text-black font-semibold leading-tight focus:outline-none focus:shadow-outline" required>
                </div>
                <div class="mb-4">
                    <label class="block text-black text-sm font-bold mb-2">วิทยากร</label>
                    <input type="text" name="speaker" class="shadow appearance-none border rounded w-full py-2 px-3 text-black leading-tight focus:outline-none focus:shadow-outline" required>
                </div>
                <div class="mb-4">
                    <label class="block text-black text-sm font-bold mb-2">สถานที่</label>
                    <input type="text" name="location" class="shadow appearance-none border rounded w-full py-2 px-3 text-black leading-tight focus:outline-none focus:shadow-outline" required>
                </div>
                <div class="mb-6">
                    <label class="block text-black text-sm font-bold mb-2">จำนวนที่นั่งทั้งหมด</label>
                    <input type="number" name="total_seats" class="shadow appearance-none border rounded w-full py-2 px-3 text-black leading-tight focus:outline-none focus:shadow-outline" required placeholder="เช่น 20">
                </div>
                <div class="mb-6">
                    ...
                    <div class="flex items-center space-x-6 bg-gray-50 p-3 rounded border">
                        ...
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="radio" name="is_active" value="0" class="w-4 h-4 text-red-600">
